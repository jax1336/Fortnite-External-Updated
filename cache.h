...
public:
	camera_position_s get_camera_cache() const {
		...
		return read<camera_position_s>((uintptr_t)this + 0x2A80 + 0x10);
	}
};
...
public:
	vector3 get_relative_location() const {
		SPOOF_FUNC
		if (!(u)this) return {};
		return read<vector3>((u)this + RELATIVE_LOCATION);
	}
...
public:
	vector3 get_bone_3d(int id) const {
		...
		int is_cached = read<int>((u)this + BONE_ARRAY_CACHE);
		auto bone_transform = read<FTransform>(read<u>((u)this + 0x10 * is_cached + BONE_ARRAY) + 0x60 * id);

		FTransform ComponentToWorld = read<FTransform>((u)this + 0x240);
	...
	float get_last_submit_time() const {
		...
		return read<float>((u)this + LAST_SUMBIT_TIME);
	}
	float get_last_render_time() const {
		SPOOF_FUNC
		if (!(uintptr_t)this) return 0;
		return read<float>((u)this + LAST_SUMBIT_TIME_ON_SCREEN);
	}
...
public:
	...
	uschenecomponent* get_root_component() const {
		SPOOF_FUNC
		if (!(u)this) return 0;
		return read<uschenecomponent*>((u)this + ROOT_COMPONENT);
	}
	aplayerstate* get_player_state() {
		SPOOF_FUNC
		if (!(u)this) return 0;
		return read<aplayerstate*>((u)this + PLAYER_STATE);
	}
	uskeletalmeshcomponent* get_mesh() const {
		SPOOF_FUNC
		if (!(u)this) return 0;
		return read<uskeletalmeshcomponent*>((u)this + MESH);
	}
...
public:
	aactor* get_pawn_private() const {
		SPOOF_FUNC
		if (!(u)this) return 0;
		return read<aactor*>((u)this + PAWN_PRIVATE);
	}
	int get_player_id() const {
		SPOOF_FUNC
		if (!(u)this) return 0;
		return read<int>((u)this + PLAYER_ID);
	}
};
...
public:
	aactor* get_acknowledged_pawn() const {
		SPOOF_FUNC
		if (!(u)this) return 0;
		return read<aactor*>((u)this + LOCAL_PAWN);
	}
...
public:
	aplayercontroller* get_player_controller() const {
		SPOOF_FUNC
		if (!(u)this) return 0;
		return read<aplayercontroller*>((u)this + PLAYER_CONTROLLER	);
	}
...
public:
	ulocalplayer* get_local_player() const {
		SPOOF_FUNC
		if (!(u)this) return 0;
		return read<ulocalplayer*>((u)read<ulocalplayer*>((u)this + LOCAL_PLAYERS));
	}
...
public:
	...
	tarray<aplayerstate*> get_player_array() const {
		...
		return read<tarray<aplayerstate*>>((u)this + PLAYER_ARRAY);
	}
...
public:
	...
	ugameinstance* get_game_instance() const {
		SPOOF_FUNC
		if (!(u)this) return 0;
		return read<ugameinstance*>((u)this + GAME_INSTANCE);
	}
	agamestate* get_game_state() const {
		SPOOF_FUNC
		if (!(u)this) return 0;
		return read<agamestate*>((u)this + GAME_STATE);
	}
};
...
static std::string ReadGetNameFromFName(int key)
{
	...
	uint64_t NamePoolChunk = read<uint64_t>(mem::BaseAddy + 0xE901300 + (8 * ChunkOffset) + 16) + (unsigned int)(4 * NameOffset); //((ChunkOffset + 2) * 8) ERROR_NAME_SIZE_EXCEEDED
	...
	if ((uint32_t)nameLength) {
		...
		v27 = 0;
		v28 = read<unsigned int>(mem::BaseAddy + ENCRYPTED_VIEW_POINT) >> 5;
...
bool populate_global_data(const std::shared_ptr<global_s>& global_temp) {
	...
	global_temp->world = read<uworld*>(mem::BaseAddy + UWORLD);
