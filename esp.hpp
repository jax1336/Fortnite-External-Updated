...
camera_position_s GetViewPoint()
{
	camera_position_s ViewPoint;
	
	uintptr_t LocationPointer = read<uintptr_t>((uintptr_t)global->world + 0x110);
	uintptr_t RotationPointer = read<uintptr_t>((uintptr_t)global->world + 0x120);

	FNRot fnRot = read<FNRot>(RotationPointer);

	ViewPoint.location = read<vector3>(LocationPointer);
	ViewPoint.rotation.x = asin(fnRot.c) * (180.0 / M_PI);
	ViewPoint.rotation.y = atan2(fnRot.a * -1, fnRot.b) * (180.0 / M_PI);
	ViewPoint.fov = read<float>((uintptr_t)global->player_controller + 0x394) * 90.f;
...
std::string username(std::uintptr_t playerstate)
{
	...
	uintptr_t pNameStructure =read<uintptr_t>(playerstate + 0xAE8);
...
//		auto fYaw = (-vAngle.y + 180.0f) * M_PI / 180.0f;  // Add 180 degrees to yaw angle
//
//		// Calculate the difference vector between the camera and the target point
//		float dx = vOrigin.x - vCamera.Location.x;
//		float dy = vOrigin.y - vCamera.Location.y;
//
//		// Rotate the difference vector according to the camera's yaw angle
//		float fsin_yaw = sinf(fYaw);
//		float fcos_yaw = cosf(fYaw);
//		float x = dx * fcos_yaw - dy * fsin_yaw;  // Correct the sign here
//		float y = dx * fsin_yaw + dy * fcos_yaw;  // Correct the sign here
//
//		// Rest of the code remains unchanged...
//
//
//	// Now x and y represent the coordinates of the target point relative to the camera's view
//
//	// Apply radar range scaling (similar to what was done in the original code)
...
void actor_loop() {
	...
	if (draw_fps) {
		...
		{
			char fpsinfo[256];
			sprintf_s(fpsinfo, (("PLAXY [PRV] ")), ImGui::GetIO().Framerate);
	...
	for (auto player : global->players) {
		...
		if (esp) {
			...
			if (player->mesh->visible()) {
				if (!rainbow)
				{
					visible_color_box = PlayerColor::BoxVisible;
					visible_color_skel = PlayerColor::SkeletonVisible;
					visible_color_Snapline = PlayerColor::SnaplineVisible;
					visible_color_Text = PlayerColor::TopTextVisible;
					visible_color_Text1 = PlayerColor::BottomTextVisible;
				...
				if (!rainbow)
				{
					...
					visible_color_box = PlayerColor::BoxNotVisible;
					visible_color_skel = PlayerColor::SkeletonNotVisible;
					visible_color_Snapline = PlayerColor::SnaplineNotVisible;
					visible_color_Text = PlayerColor::TopTextNotVisible;
					visible_color_Text1 = PlayerColor::BottomTextNotVisible;
			...
			if (!inlobby && testdbg) {
				...
				uint64_t player_weapon = read<uint64_t>(reinterpret_cast<uint64_t>(player->player) + 0x948);
				char const* string1;
				...
				auto bIsSliding = read<bool>(reinterpret_cast<uint64_t>(player->player) + 0x753); //BoolProperty Class FortniteGame.FortPawn.bIsSliding -> 0x753 (Mask: 8)
				auto bIsJumping = read<bool>(reinterpret_cast<uint64_t>(player->player) + 0x940); //BoolProperty Class FortniteGame.FortPawn.bIsJumping -> 0x940 (Mask: 1)
				...
				if (player->player_distance > 310)
				{
					less = 50;
						...
						DrawString(18, (head_2d.x + 80) - (TextSize.x / 2), (head_2d.y + 35 - less), visible_color_Text, false, true, text_ammo_count);
					
			

						DrawString(18, (head_2d.x + 80) - (TextSize.x / 2), (head_2d.y + 55 - less), visible_color_Text, false, true, text_ammo_count2);

						DrawString(18, (head_2d.x + 80) - (TextSize.x / 2), (head_2d.y + 75 - less), visible_color_Text, false, true, final.c_str());
						DrawString(18, (head_2d.x + 80) - (TextSize.x / 2), (head_2d.y + 95 - less), visible_color_Text, false, true, text_ammo_count4);
						DrawString(18, (head_2d.x + 80) - (TextSize.x / 2), (head_2d.y + 115 - less), visible_color_Text, false, true, text_ammo_count3);
			...
			if (draw_distance) {
				...
				DrawString(18, (base_2d.x) - (TextSize.x / 2), (base_2d.y + 5), visible_color_Text, false, true, final.c_str());
			...
			if (draw_username) {
				...
				if (global->local_pawn == NULL)
				{
					DrawString(20, head_2d.x - 6, head_2d.y - 13, visible_color_Text, true, true, name.c_str());
				}
				else
				{
					DrawString(20, head_2d.x - 20, head_2d.y - 13, visible_color_Text, true, true, name.c_str());
				}
			}
		...
		if (visibble)
		{
			...
			if (targetcirlce && dist < fov && HeadBone or NeckBbone or ChestBone or PelvisBone) {
				...
				draw_list->AddText(ImVec2(screen_width / 2 - 65, screen_height / 2 - 90), ImColor(PlayerColor::accentcolor), test12.c_str());
